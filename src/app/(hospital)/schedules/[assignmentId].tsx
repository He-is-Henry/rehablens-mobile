...
export default function HospitalScheduleManagementScreen() {
  ...
  const { data: assignment, loading: loadingAssignment } = useHospitalQuery.assignmentById(assignmentId);
  const { data: link, loading: loadingPatient } = useHospitalQuery.patientByLinkId(linkId ?? '');
  const {
    data: schedules,
    loading: loadingSchedules,
    refreshData: refreshSchedules,
  } = useHospitalQuery.schedules(assignmentId);
  ...
  const monthlyGroups = useMemo(() => {
    ...
    schedules.forEach((slot) => {
      ...
      const key = slot.scheduledDate.substring(0, 7); const label = d.toLocaleDateString(undefined, { month: 'long', year: 'numeric' });
  ...
  const handleCreateSchedules = async (
    ...
    await createHospitalSchedules(assignmentId, entries);
    refreshSchedules();
  };
  ...
  const handleUpdateSlot = async (
    ...
    const { minSessions, maxSessions, scheduledDate } = data
    await updateHospitalSchedule(slotId, { minSessions, maxSessions, scheduledDate });
    refreshSchedules();
  };
  ...
  const handleDeleteSlot = async (slotId: string) => {
    Alert.alert('Delete Slot', 'Are you sure you want to delete this scheduled date?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await deleteHospitalSchedule(slotId);
            refreshSchedules();
          } catch (err: any) {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Patient & Exercise Context Header */}
      <View style={styles.contextCard}>
        {loadingPatient || loadingAssignment ? (
          <ActivityIndicator size="small" color={colors.primary} />
        ) : (
          <>
            <View style={styles.contextRow}>
              <Text style={styles.patientName}>{patient?.name ?? 'Patient'}</Text>
              {patient?.customId && <Text style={styles.patientId}>ID: {patient.customId}</Text>}
            </View>
      ...
      {/* Schedule List (Monthly Collapsible Accordions) */}
      {loadingSchedules ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySub}>
            Tap "Generate" above to generate scheduled exercise days for this patient.
          </Text>
        </View>
      ) : (
          ...
          {monthlyGroups.map((group) => {
            ...
            return (
              <View key={group.key} style={styles.monthSection}>
                ...
                {/* Collapsible Content (Uncollapsed by default) */}
                {!isCollapsed && (
